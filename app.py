...
if hasattr(st, "query_params"):
    ...
    def get_query_param(key, default=None):
        ...
        return params[key][0] if key in params else default
    def set_query_param(key, value):
...
def render_nav_bar():
    ...
    project_id = get_query_param("project_id", None)

    if project_id:
        db = SessionLocal()
        try:
            pid_int = int(project_id)
            proj = db.query(Project).filter(Project.id == pid_int).first()
    ...
    for p in pages:
        ...
        # Using a link that sets the query parameter.
            ...
            if st.form_submit_button("Create Task"):
                if t_title:
                    try:
                        ...
                        db.add(new_task)
                        db.commit()
                        st.success(f"Task '{t_title}' created!")
                        app_rerun()
                    except Exception as e:
                        db.rollback()
...
if __name__ == "__main__":
    render_nav_bar()
    # Router
    pid = get_query_param("project_id", None)
    page = get_query_param("page", "home")
